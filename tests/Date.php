 ...
 */
class FWS_Tests_Date extends FWS_Test_Case
{
	/**
	...
	public function testTo_date()
	{
		...
		$d = new FWS_Date();
		self::assert_equals($d->to_date(),'<b>0 seconds ago</b>');
		$d->modify('-1minute');
		self::assert_equals($d->to_date(),'<b>1 minute ago</b>');
		$d->modify('+1second');
		self::assert_equals($d->to_date(),'<b>59 seconds ago</b>');
		...
		self::assert_equals($d->to_date(),'<b>Today</b>, 12:00 AM');
		
		// local 'today 23:59'
		$d = new FWS_Date(array(23,59,59),FWS_Date::TZ_USER,FWS_Date::TZ_USER);
		self::assert_equals($d->to_date(),'<b>Today</b>, 11:59 PM');
		
		// local 'today 0:00' viewed from GMT
		$d = new FWS_Date(array(0,0,0),FWS_Date::TZ_USER,FWS_Date::TZ_GMT);
		if($offset == 7200)
			self::assert_equals($d->to_date(),'Yesterday, 10:00 PM');
		else
			self::assert_equals($d->to_date(),'Yesterday, 11:00 PM');
		
		// local 'today 23:59' viewed from GMT
		$d = new FWS_Date(array(23,59,59),FWS_Date::TZ_USER,FWS_Date::TZ_GMT);
		if($offset == 7200)
			self::assert_equals($d->to_date(),'<b>Today</b>, 9:59 PM');
		else
			self::assert_equals($d->to_date(),'<b>Today</b>, 10:59 PM');
		
		// GMT 'today 0:00' viewed from local
		$d = new FWS_Date(array(0,0,0),FWS_Date::TZ_GMT,FWS_Date::TZ_USER);
		if($offset == 7200)
			self::assert_equals($d->to_date(),'<b>Today</b>, 2:00 AM');
		else
			self::assert_equals($d->to_date(),'<b>Today</b>, 1:00 AM');
		
		// GMT 'today 23:59' viewed from local
		$d = new FWS_Date(array(23,59,59),FWS_Date::TZ_GMT,FWS_Date::TZ_USER);
		if($offset == 7200)
			self::assert_equals($d->to_date(),'Tomorrow, 1:59 AM');
		else
			self::assert_equals($d->to_date(),'Tomorrow, 12:59 AM');
	...
	public function testTo_timestamp()
	{
		...
		self::assert_equals($d->to_timestamp(),$gts);
		...
		self::assert_equals($d->to_timestamp(),$ts);
	...
	public function test__construct()
	{
		...
		echo $d->to_format($df)."\n";
		self::assert_equals($d->to_format($df),gmdate($df,$ts));
		
		$d = new FWS_Date($ts,FWS_Date::TZ_GMT,FWS_Date::TZ_USER);
		echo $d->to_format($df)."\n";
		self::assert_equals($d->to_format($df),date($df,$ts));
		...
		echo $d->to_format($df)."\n";
		self::assert_equals($d->to_format($df),gmdate($df,strtotime('@'.$ts.' -1hour')));
		...
		echo $d->to_format($df)."\n";
		self::assert_equals($d->to_format($df),date($df,strtotime('@'.$ts.' -1hour')));
		
		
		// now the same, but using the date-array instead of the timestamp
		$d = new FWS_Date($date,FWS_Date::TZ_GMT,FWS_Date::TZ_GMT);
		echo $d->to_format($df)."\n";
		self::assert_equals($d->to_format($df),gmdate($df,$gts));
		
		$d = new FWS_Date($date,FWS_Date::TZ_GMT,FWS_Date::TZ_USER);
		echo $d->to_format($df)."\n";
		self::assert_equals($d->to_format($df),date($df,$gts));
		...
		$d = new FWS_Date($ts,FWS_Date::TZ_USER,FWS_Date::TZ_GMT);
		echo $d->to_format($df)."\n";
		self::assert_equals($d->to_format($df),gmdate($df,strtotime('@'.$ts.' -2hour')));
		
		// the same as above, but now interpreted in user-time, therefore 2 hour less
		// compared to assuming an GMT-timestamp, but using date instead of gmdate.
		$d = new FWS_Date($ts,FWS_Date::TZ_USER,FWS_Date::TZ_USER);
		echo $d->to_format($df)."\n";
		self::assert_equals($d->to_format($df),date($df,strtotime('@'.$ts.' -2hour')));
